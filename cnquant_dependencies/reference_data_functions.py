...
import logging
def load_reference_data_annotation(csv_path: str | Path, logger: logging.Logger = logging.getLogger(__name__), minimum_number_of_reference_samples: int = 5) -> dict[ArrayType, list[str]]:
    """
    ...
    reference_450k_reference_samples = reference_dataframe.filter(pl.col("array_type") == ArrayType.ILLUMINA_450K.value).select("Sentrix_id").to_series().to_list()
    EPIC_v1_reference_samples = reference_dataframe.filter(pl.col("array_type") == ArrayType.ILLUMINA_EPIC.value).select("Sentrix_id").to_series().to_list()
    EPIC_v2_reference_samples = reference_dataframe.filter(pl.col("array_type") == ArrayType.ILLUMINA_EPIC_V2.value).select("Sentrix_id").to_series().to_list()
    msa48_reference_samples = reference_dataframe.filter(pl.col("array_type") == ArrayType.ILLUMINA_MSA48.value).select("Sentrix_id").to_series().to_list()
    ...
    for key in reference_data_sentrix_ids_and_types.keys():
        if len(reference_data_sentrix_ids_and_types[key]) < minimum_number_of_reference_samples:
            logger.warning(
